...
def main(args):
    ...
    if "sam" in args.optimizer.lower():
        assert args.gradient_accumulation == 1, "SAM requires gradient_accumulation to be 1"
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        if not layer_wise_flag: # layer-wise updation is done during backward; requires gradient_accumulation equals 1
            if args.optimizer.lower() in ["sam"]:
                optimizer.first_step(zero_grad=True)
                loss = model(**batch, labels=labels).loss
                loss.backward()
                optimizer.second_step(zero_grad=True)
            elif args.optimizer.lower() in ["function_sam"]:
                def closure():
                    # optimizer.zero_grad()
                    # loss = model(**batch, labels=labels).loss
                ...
                # # 在 closure 里定义 network_fn
                # def network_fn():
                #     with torch.enable_grad():
                #         logits=model(**batch).logits
                #         shift_logits = logits[..., :-1, :].contiguous()
                #         shift_logits = shift_logits.view(-1, model.config.vocab_size)
                #         assert shift_logits.requires_grad, "🚨 ERROR: logits does not require gradients!"
                #         return shift_logits  # 计算当前 batch 的 logits

                # 获取模型参数名称
                # param_names = [name for name, _ in model.named_parameters()]

                # def network_fn(params):
                #     with torch.enable_grad():
                #         # ✅ 用 functional_call 让 model 真的使用 params
                #         param_dict = {name: p for name, p in zip(param_names, params)}
                #         logits = torch.func.functional_call(model, param_dict, batch).logits  # ✅ 确保 logits 用了新的 params
                #         shift_logits = logits[..., :-1, :].contiguous()
                #         shift_logits = shift_logits.view(-1, model.config.vocab_size)
                #         assert shift_logits.requires_grad, "🚨 ERROR: logits does not require gradients!"
                #         return shift_logits

                # optimizer.defaults["network_fn"] = network_fn  # 动态更新 network_fn
        ...
        if local_step > args.gradient_accumulation and update_step % args.save_every == 0 and global_rank == 0:
            ...
            if args.single_gpu:
                model.save_pretrained(current_model_directory, max_shard_size='100GB', from_pt=True)
                saving_model_weight(model, f"{current_model_directory}/pytorch_model.bin", args)
            else:
